...
const StartEarning = () => {
  ...
  const { operatorAddress, sendMessage, isAuthenticated: isHotWalletAuthenticated } = useHotWallet();
  const [isLoading, setIsLoading] = useState(false);

  // This function would be passed to createAppSessionMessage
  // It needs access to the user's wallet signer
  const userMessageSigner: MessageSigner = async (payload: any) => {
  ...
  const initiateOffChainDeposit = async () => {
    ...
    try {
        ...
        const allocations = [
            ...
            { participant: operatorAddress as Address, asset: 'PYUSD', amount: '0' },
        ];
        ...
        console.log(`[USER] Sent app session request to hot wallet for ${amount} PYUSD.`);
        alert(`Your deposit of ${amount} PYUSD has been initiated instantly! The hot wallet will now process the on-chain transaction in the background.`);
    ...
    <>
      <Card className="w-full max-w-lg items-center mx-auto">
        <CardHeader className="w-full">
          <CardTitle className="text-center">Start Earning with PYUSD</CardTitle>
        </CardHeader>
          ...
          <div className="grid gap-3 w-full text-left">
            ...
            />
             <p className="text-sm text-muted-foreground">
                Instantly deposit PYUSD off-chain. Our hot wallet will handle the on-chain staking in the background.
            </p>
          </div>
        </CardContent>
        <CardFooter className="flex-col gap-y-5">
          <div className="grid gap-3">
            <Button
              ...
              disabled={!amount || parseFloat(amount) <= 0 || isLoading || !isHotWalletAuthenticated}
            >
              {isLoading 
                ? <CircleAlertIcon className="size-5 animate-spin" />
                : isHotWalletAuthenticated ? "Deposit & Earn (Instant)" : "Hot Wallet Connecting..."
              }
            </Button>
          </div>
        </CardFooter>
